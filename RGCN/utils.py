...
############################################################################################################################
#
#Note : entities.dict must in an order that "course" entities always 
# in first to calculate filtered MRR among only courses
#
############################################################################################################################
...
def perturb_o_and_get_filtered_rank(embedding, w, s, r, o, test_size, triplets_to_filter):
    ...
    for idx in range(test_size):
        ...
        indices_f = [x for x in indices.numpy() if x < 4477] # 4476 is the number of course entities. Select only course entities
...
def perturb_s_and_get_filtered_rank(embedding, w, s, r, o, test_size, triplets_to_filter):
    ...
    for idx in range(test_size):
        ...
        scores = torch.sigmoid(torch.sum(emb_triplet, dim=1))
        _, indices = torch.sort(scores, descending=True)
        indices_f = [x for x in indices.numpy() if x < 4477] # 4477 is the number of course entities. Select only course entities
        for i in range(len(indices_f)):
