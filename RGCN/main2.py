...
from create_data import load_from_file

def train(train_triplets, model, batch_size, split_size, negative_sample, reg_ratio, num_entities, num_relations): #use_cuda
...
def main(args):
    ...
    entity2id, relation2id, train_triplets, valid_triplets, test_triplets = load_data('/data/workspace/holly0015/test_project1/project2/RGCN/경영data')
    ...
    for epoch in trange(1, (args.n_epochs + 1), desc='Epochs', position=0):
        ...
        loss = train(train_triplets, model, batch_size=args.graph_batch_size,
        ...
        if epoch % args.evaluate_every == 0:
            ...
            if valid_mrr > best_mrr:
                best_mrr = valid_mrr
                torch.save(
                    {'state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'epoch': epoch,
    ...
    history.to_csv("history_0626.txt")
   ...
   # test_mrr = test(test_triplets, model, train_triplets, test_graph, valid_triplets)
    ...
    # predict
    torch.save(model, os.path.join('/data/workspace/holly0015/test_project1/project2/RGCN/output', 'modelnew.pt'))
    model = torch.load('/data/workspace/holly0015/test_project1/project2/RGCN/output/modelnew.pt')
    # prediction function
    entity_embedding = model(test_graph.entity, test_graph.edge_index, test_graph.edge_type, test_graph.edge_norm)
    from utils import get_score_all
    list_123 = get_score_all(entity_embedding, model.relation_embedding, train_triplets, valid_triplets, test_triplets,
                              entity2id)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--graph-batch-size", type=int, default=4000)
    parser.add_argument("--graph-split-size", type=float, default=0.5)
    parser.add_argument("--negative-sample", type=int, default=1)
